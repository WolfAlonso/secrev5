    ...
    <aside id="sidebar-main" style="transform: translateX(-120%);" class="fixed left-4 top-4 bottom-4 w-72 bg-slate-950/95 border border-slate-800/80 backdrop-blur-2xl rounded-3xl z-50 flex flex-col justify-between p-4 shadow-[0_0_40px_rgba(6,182,212,0.25)] transition-transform duration-300 ease-in-out">
      ...
      <div class="space-y-4 overflow-y-auto pr-1">
        ...
        <nav class="space-y-1.5 text-xs font-semibold">
          ...
          ${puedeVerAgendas ? `
            <div class="space-y-1">
              <button onclick="toggleSubmenu('sub-despacho')" class="w-full flex items-center justify-between px-3.5 py-3 rounded-2xl transition ${esAgendas ? 'bg-cyan-950/65 text-cyan-300 border border-cyan-500/50 shadow-[0_0_15px_rgba(6,182,212,0.3)] font-bold' : 'text-slate-400 hover:bg-slate-900/60 hover:text-white'}">
                <div class="flex items-center space-x-3">
                  <i class="fa-solid fa-folder text-sm w-5 text-center"></i>
                  <span>Despacho de Concejo</span>
                </div>
                <i class="fa-solid fa-chevron-down text-[10px] transition-transform ${esAgendas ? 'rotate-180' : ''}" id="arrow-sub-despacho"></i>
              </button>
              <div id="sub-despacho" class="${esAgendas ? '' : 'hidden'} pl-8 space-y-1 text-[11px]">
                <a href="agendas.html" class="block py-1.5 px-3 rounded-xl ${esAgendasConcejo ? 'text-cyan-300 font-bold bg-cyan-500/10' : 'text-slate-400 hover:text-white'}">Agendas de Concejo</a>
                <a href="agendas.html?vista=convocatorias" class="block py-1.5 px-3 rounded-xl ${esConvocatorias ? 'text-cyan-300 font-bold bg-cyan-500/10' : 'text-slate-400 hover:text-white'}">Convocatorias</a>
              </div>
            </div>
          ` : ''}
