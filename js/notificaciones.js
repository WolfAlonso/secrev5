...
// v2: Cerrar al clic fuera + rol case-insensitive
// ============================================================================
...
function renderizarBotonNotificaciones() {
  ...
  contenedor.innerHTML = `
    <div class="relative">
      <button id="btn-notificaciones" onclick="togglePanelNotificaciones(event)" class="relative bg-slate-900/80 border border-cyan-500/40 text-cyan-300 p-3 rounded-2xl shadow-[0_0_15px_rgba(6,182,212,0.3)] backdrop-blur-xl hover:bg-cyan-500/20 hover:text-white transition-all duration-300 group">
  ...
  try {
    ...
    if (rol === 'admin' || rol === 'secretario') {
      ...
      if (error) {
        ...
        lista.innerHTML = '<p class="text-slate-500 text-center py-4">Error al cargar.</p>';
        return;
      }
      ...
      if (accesos && accesos.length > 0) {
        ...
        htmlNotifs = accesos.map(a => {
          const fechaRaw = a.fecha_hora || a.fecha || a.created_at;
          const dt = fechaRaw ? new Date(fechaRaw) : new Date();
          const horaFmt = dt.toLocaleTimeString('es-GT', { hour: '2-digit', minute: '2-digit' });
          const esIngreso = a.tipo_evento === 'INICIO_SESION';
          const ipInfo = a.ip_address ? `<p class="text-[9px] text-cyan-400 font-mono">IP: ${a.ip_address}</p>` : '';
